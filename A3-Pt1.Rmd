...
climate_happening <- climate_opinion_address |>
    read.xlsx(
...
write_parquet(climate_happening, "climate_happening.parquet")
...
covid_2020 <- covid_cases|>
  ...
  summarise(total_cases = max(total_cases, na.rm=TRUE))|> rename(country = location)
...
aware_pop<- inner_join(pop_data,aware_fixed_country_match, by=join_by(country))|> rename(total_population=SP.POP.TOTL)|>
...
covid_2020_match_country<- covid_2020|> mutate(country=case_when(
  country=="Congo"~"Congo, Dem. Rep.", country=="Egypt"~"Egypt, Arab Rep.", country=="Hong Kong"~"Hong Kong SAR, China", country=="Laos"~"Lao PDR", country=="Slovakia"~"Slovak Republic", country=="South Korea"~"Korea, Rep.", country=="Turkey"~"Turkiye", country=="Vietnam"~"Viet Nam", country=="Yemen"~"Yemen, Rep.", TRUE~country
))
...
aware_covid<- inner_join(aware_pop,covid_2020_match_country,by=join_by(country))|> mutate(covid_per_capita=total_cases/total_population)
